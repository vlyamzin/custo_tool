...
export function useFileUpload(type: keyof PlatformCustomization): FileUploadHook {
  ...
  function uploadFile(info: UploadChangeParam): void {
    ...
    if (info?.file.status === 'done') {
      const logo = config.customization[type].find(i => i.locale === config.selectedLocale);
      logo
        ? logo.value = info.file.response.path
        : config.customization[type].push({locale: config.selectedLocale as string, value: info.file.response.path});
      setConfig({...config});
    }
